...
module.exports = {
  ...
  googleMapsTest: function(req, res) {
    ...
    var lat = 37.774933;
    var long = -122.419411;
    // var googleMapsURL = 'http://maps.googleapis.com/maps/api/geocode/json?latlng=' + lat + ',' + long;
    // var googleMapsURL = 'http://maps.googleapis.com/maps/api/geocode/json?latlng=37.774933,-122.419411';
    var googleMapsURL = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' +lat + ',' + long + '&result_type=country&key=AIzaSyC2XH3yPRy-DfzFgkcl1JaVsFeC-qm0T0E';
    https.get(googleMapsURL, function(response) {
      var body = '';
      response.on('data', function(d) {
        body += d;
      });
      response.on('end', function() {
        console.log('response from googleMaps');
        ...
        res.send(body.results[0].address_components[0].short_name);
      });
      }
    );
  },
  ...
  groupByAttempt: function(req, res) {
    //this works, but we are not using it either. 
  ...
  insertCountryToDb: function(req, res) {
    //This works (not fully hooked up yet, but it works), but we are not using it. 
